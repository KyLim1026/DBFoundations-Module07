...
-- 2020-12-02,Kyoungmin Lim,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_KyoungminLim')
	 Begin 
	  Alter Database [Assignment07DB_KyoungminLim] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_KyoungminLim;
	 End
	Create Database Assignment07DB_KyoungminLim;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_KyoungminLim;
...
-- <Put Your Code Here> --
Select ProductName, [InventoryDate] = DateName(mm, InventoryDate)+','+DateName(yyyy, InventoryDate),[InventoryCount]=[Count]
...
AS
Select Top 1000000
ProductName, [InventoryDate] = DateName(mm, InventoryDate)+','+DateName(yyyy, InventoryDate),[InventoryCount]=[Count]
From vProducts as P Inner Join vInventories as I
On P.ProductID = I. ProductID
Order by ProductName, month(InventoryDate), [Count];
Go
...
As
Select TOP 10000000 
CategoryName,[InventoryDate] = DateName(mm, InventoryDate)+','+DateName(yyyy, InventoryDate), InventoryCountByCategory=SUM([Count])
...
As
Select ProductName, InventoryDate, InventoryCount, [PreviousMonthCount]= IIF(Year(InventoryDate) = 1996, 0, ISNull(Lag(sum(InventoryCount)) over (order by ProductName, Month(InventoryDate)),0))
...
-- <Put Your Code Here> --
Create Function fProductInventoriesWithPreviousMonthCountsWithKPIs(@KPICount int)
Returns Table
As
Return (
Select TOP 1000000
...
Where CountVsPreviousCountKPI=@KPICount 
